...
class CourseRestorer
{
    ...
    public function __construct($course)
    {
        ...
        $forceImport = (api_get_setting('lp.allow_import_scorm_package_in_course_builder') === 'true');
        if ($forceImport) {
    ...
    public function restore_documents(
        ...
        foreach ($resources[RESOURCE_DOCUMENT] as $id => $document) {
            ...
            //$path = api_get_path(SYS_COURSE_PATH).$this->course->destination_path.'/';
            if ($respect_base_content === false && $session_id) {
            ...
            if ($document->file_type == FOLDER) {
                ...
                if (file_exists($path . $document->path)) {
                        ...
                        case FILE_RENAME:
                            ...
                            if (!empty($session_id)) {
                                ...
                                if (is_dir($orig_base_path)) {
                                    ...
                                    if (
                                        isset($_SESSION['orig_base_foldername']) &&
                    ...
                    if (
                        is_file($this->course->backup_path . '/' . $document->path) &&
            ...
            if (
                preg_match('/^quiz-.*$/', $document->title) &&
    ...
    public function restore_forums($sessionId = 0)
    {
        if ($this->course->has_resources(RESOURCE_FORUM)) {
            ...
            foreach ($resources[RESOURCE_FORUM] as $id => $forum) {
                ...
                $cat_id = '';
                if (
                    isset($this->course->resources[RESOURCE_FORUMCATEGORY]) &&
                ...
                if ($new_id) {
                    ...
                    $forum_topics = 0;
                    if (
                        isset($this->course->resources[RESOURCE_FORUMTOPIC]) &&
        ...
        if ($this->course->has_resources(RESOURCE_EVENT)) {
            ...
            foreach ($resources[RESOURCE_EVENT] as $id => $event) {
                ...
                if (!empty($this->course->orig)) {
                    ...
                    if (
                        file_exists($origin_path . $attachment_event->path) &&
        ...
        if ($this->course->has_resources(RESOURCE_ANNOUNCEMENT)) {
            ...
            foreach ($resources[RESOURCE_ANNOUNCEMENT] as $id => $announcement) {
                ...
                if (!empty($this->course->orig)) {
                    ...
                    // get the info of the file
                    if (
                        !empty($announcement->attachment_path) &&
    ...
    public function restore_quizzes(
        ...
        if ($this->course->has_resources(RESOURCE_QUIZ)) {
            ...
            foreach ($resources[RESOURCE_QUIZ] as $id => $quiz) {
                ...
                if (!empty($quiz->sound)) {
                    if (
                        isset($this->course->resources[RESOURCE_DOCUMENT][$quiz->sound]) &&
                ...
                if ($id != -1) {
                    ...
                    global $_custom;
                    if (
                        isset($_custom['exercises_clean_dates_when_restoring']) &&
                    ...
                    $allow = (api_get_setting('exercise.allow_notification_setting_per_exercise') === 'true');
                    if ($allow) {
    ...
    public function restore_quiz_question($id)
    {
        ...
        if (is_object($question)) {
            ...
            $imageNewId = '';
            if (
                preg_match('/^quiz-.*$/', $question->picture) &&
    ...
    public function restore_learnpath_category($sessionId = 0, $baseContent = false)
    {
        ...
        if (
            isset($this->tool_copy_settings['learnpath_category']) &&
            isset($this->tool_copy_settings['learnpath_category']['reuse_existing']) &&
            $this->tool_copy_settings['learnpath_category']['reuse_existing'] === true
        ) {
            $reuseExisting = true;
        }
        ...
        if ($this->course->has_resources(RESOURCE_LEARNPATH)) {
            ...
            foreach ($resources[RESOURCE_LEARNPATH] as $id => $lp) {
                ...
                if (!empty($lp->preview_image)) {
                    ...
                    if (
                        file_exists($origin_path . $lp->preview_image) &&
                ...
                if (isset($this->tool_copy_settings['learnpaths'])) {
                    if (
                        isset($this->tool_copy_settings['learnpaths']['reset_dates']) &&
                ...
                $lp->published_on = isset($lp->published_on) && $lp->published_on === '0000-00-00 00:00:00' ? null : $lp->published_on;
    ...
    public function get_new_id($tool, $ref)
    {
        ...
        if (
            isset($this->course->resources[$tool][$ref]) &&
                    ...
                    foreach ($obj->categories as $cat) {
                        ...
                        if (!empty($cat->links)) {
                            ...
                            foreach ($cat->links as $link) {
                                ...
                                if (
                                    $this->course->has_resources($type) &&
    ...
    public function restore_assets()
    {
        if ($this->course->has_resources(RESOURCE_ASSET)) {
            ...
            foreach ($resources[RESOURCE_ASSET] as $asset) {
                if (
                    is_file($this->course->backup_path . '/' . $asset->path) &&
