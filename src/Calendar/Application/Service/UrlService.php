...
class UrlService
{
    ...
    final public const string REPLACE_ENCODE = '+/=';

    final public const string REPLACE_DECODE = '._-';
    ...
    protected function getEncoded(CalendarImage $calendarImage, bool $short = false): string
    {
        ...
        return match ($calendarImage->getMonth()) {
            0 => self::encode(BaseController::CONFIG_APP_CALENDAR_INDEX, [
                'hash' => $short ? $calendar->getUser()->getIdHashShort() : $calendar->getUser()->getIdHash(),
                'userId' => $calendar->getUser()->getId(),
                'calendarId' => $calendar->getId(),
            ], false, true),
            default => self::encode(BaseController::CONFIG_APP_CALENDAR_DETAIL, [
                'hash' => $short ? $calendar->getUser()->getIdHashShort() : $calendar->getUser()->getIdHash(),
                'userId' => $calendar->getUser()->getId(),
