...
use App\Access\Domain\Entity\AccessUrlRelSession;
use App\CoreBundle\Entity\Course\Course;
use App\CoreBundle\Entity\Listener\SessionListener;
...
use App\CoreBundle\State\UserSessionSubscriptionsStateProvider;
...
class Session implements ResourceWithAccessUrlInterface, Stringable
{
    ...
    #[ORM\Column(name: 'position', type: 'integer', nullable: false, options: [
        'default' => 0,
    ])]
    ...
    #[ORM\Column(name: 'notify_boss', type: 'boolean', options: [
        'default' => false,
    ])]
    ...
    public function removeUserCourseSubscription(User $user, Course $course): void
    {
        foreach ($this->sessionRelCourseRelUsers as $i => $sessionRelUser) {
            if (
                $sessionRelUser->getCourse()->getId() === $course->getId()
                && $sessionRelUser->getUser()->getId() === $user->getId()
            ) {
                if ($this->sessionRelCourseRelUsers[$i]->getStatus() === self::STUDENT) {
    ...
    {
        if ($this->getSessionRelCourseRelUsers()->count() !== 0) {
            $criteria = Criteria::create()
                ->where(
                    Criteria::expr()->eq('user', $subscription->getUser())
                )
    ...
    public function isAvailableByDurationForUser(User $user): bool
    {
        ...
        if ($user->getTrackECourseAccess()->count() === 0) {
            return true;
        }
