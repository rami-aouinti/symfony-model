...
class AnonymousUserSubscriber implements EventSubscriberInterface
{
    ...
    public function onKernelRequest(RequestEvent $event): void
    {
        ...
        if ($anonymousUserId !== null) {
            ...
            $existingLogin = $trackLoginRepository->findOneBy([
                'userIp' => $userIp,
    ...
    private function getOrCreateAnonymousUserId(string $userIp): ?int
    {
        $userRepository = $this->entityManager->getRepository(User::class);
        $trackLoginRepository = $this->entityManager->getRepository(TrackELogin::class);
        $anonymousAutoProvisioning = $this->settingsManager->getSetting('security.anonymous_autoprovisioning') === 'true';
        ...
        if ($maxAnonymousUsers === 0) {
            $maxAnonymousUsers = self::MAX_ANONYMOUS_USERS;
        }
        $anonymousUsers = $userRepository->findBy([
            'status' => User::ANONYMOUS,
        ], [
            'registrationDate' => 'ASC',
        ]);
        ...
        foreach ($anonymousUsers as $user) {
            $loginRecord = $trackLoginRepository->findOneBy([
                'userIp' => $userIp,
                'user' => $user,
            ]);
            if ($loginRecord) {
    ...
    private function createAnonymousUser(): User
    {
        ...
        if ($this->settingsManager->getSetting('profile.login_is_email') === 'true') {
            $uniqueId = $email;
        }
