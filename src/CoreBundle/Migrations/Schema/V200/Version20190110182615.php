...
class Version20190110182615 extends AbstractMigrationChamilo
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if ($table->hasColumn('asset_id') === false) {
        ...
        if ($table->hasColumn('accumulate_work_time') === false) {
        ...
        if ($table->hasColumn('id') === false) {
            $this->addSql('ALTER TABLE c_lp DROP id');
        }

        if ($table->hasIndex('session') === false) {
            // $this->addSql('DROP INDEX session ON c_lp');
        }

        $this->addSql('UPDATE c_lp SET category_id = NULL WHERE category_id = 0');
        if ($table->hasForeignKey('FK_F67ABBEB12469DE2') === false) {
            $this->addSql(
        ...
        if ($table->hasColumn('session_id') === false) {
        ...
        if ($table->hasForeignKey('FK_90A0FC071BAD783F') === false) {
            $this->addSql(
        ...
        if ($table->hasIndex('UNIQ_90A0FC071BAD783F') === false) {
            $this->addSql('CREATE UNIQUE INDEX UNIQ_90A0FC071BAD783F ON c_lp_category (resource_node_id)');
        }

        if ($schema->getTable('c_lp')->hasIndex('IDX_F67ABBEB12469DE2') === false) {
        ...
        if ($table->hasForeignKey('FK_CCC9C1ED68DFD1EF') === false) {
            $this->addSql(
        ...
        if ($schema->hasTable('c_lp_rel_usergroup') === false) {
            $this->addSql(
