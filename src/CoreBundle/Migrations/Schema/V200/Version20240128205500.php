    ...
    public function up(Schema $schema): void
    {
        ...
        foreach ($q->toIterable() as $userEntity) {
            ...
            foreach ($files as $file) {
                ...
                $certificate = $gradebookCertificateRepo->findOneBy([
                    'pathCertificate' => '/' . $originalTitle,
                ]);
                if (!$certificate) {
                ...
                $catId = $certificate->getCategory() !== null ? $certificate->getCategory()->getId() : 0;
                $newTitle = hash('sha256', $id . $catId) . '.html';

                $existingFile = $personalRepo->findOneBy([
                    'title' => $newTitle,
                ]);
                if ($existingFile) {
