...
class Version20170904173000 extends AbstractMigrationChamilo
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if ($table->hasIndex('IDX_CE06532412469DE2') === false) {
        ...
        if ($table->hasIndex('IDX_C5D3D49FA76ED395') === false) {
            $this->addSql('CREATE INDEX IDX_C5D3D49FA76ED395 ON c_group_rel_user (user_id)');
        }
        if ($table->hasIndex('IDX_C5D3D49FFE54D947') === false) {
        ...
        if ($table->hasIndex('IDX_F6FF71ABA76ED395') === false) {
            $this->addSql('CREATE INDEX IDX_F6FF71ABA76ED395 ON c_group_rel_tutor (user_id)');
        }
        if ($table->hasIndex('IDX_F6FF71ABFE54D947') === false) {
        ...
        if ($schema->getTable('c_group_info')->hasForeignKey('FK_CE06532412469DE2') === false) {
