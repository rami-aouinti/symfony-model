...
class Version20170626122900 extends AbstractMigrationChamilo
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if ($table->hasColumn('timezone') === false) {
            $this->addSql('ALTER TABLE user ADD timezone VARCHAR(64) NOT NULL');
        }

        if ($table->hasColumn('confirmation_token') === false) {
        ...
        if ($table->hasForeignKey('FK_880E0D76A76ED395') === false) {
            $this->addSql(
        ...
        if ($table->hasForeignKey('FK_BD241818A76ED395') === false) {
            $this->addSql(
        ...
        if ($table->hasForeignKey('FK_DBF650A8A76ED395') === false) {
            $this->addSql(
