...
final class Version20230720143000 extends AbstractMigrationChamilo
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $query = $this->entityManager->createQuery('SELECT s.selectedValue FROM App\CoreBundle\Entity\SettingsCurrent s WHERE s.variable = :variable')
            ->setParameter('variable', $variable);
        $result = $query->getOneOrNullResult();
        $splitUsersUploadDirectory = $result['selectedValue'] ?? 'false';

        if ($splitUsersUploadDirectory === 'true') {
