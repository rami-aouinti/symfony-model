...
use App\CoreBundle\Repository\TrackEOnlineRepository;
...
class SocialController extends AbstractController
{
    ...
    public function getLegalStatus(
        ...
        $allowTermsConditions = $settingsManager->getSetting('registration.allow_terms_conditions') === 'true';
    ...
    public function deleteLegal(Request $request, TranslatorInterface $translator): JsonResponse
    {
        ...
        if (!$userId) {
            return $this->json([
                'error' => $translator->trans('User ID not provided'),
        ...
        return $this->json([
            'success' => true,
            'message' => $translator->trans('Legal acceptance revoked successfully.'),
        ]);
    }
        ...
        try {
            switch ($action) {
                case 'send_invitation':
                    ...
                    if (!$result) {
                        return $this->json([
                            'error' => 'Invitation already exists or could not be sent',
                        ]);
                    }
    ...
    public function uploadGroupPicture(
        ...
        return new JsonResponse([
            'success' => 'Group and image saved successfully',
        ], Response::HTTP_OK);
    }
    ...
    public function checkTermsRestrictions(
        ...
        if (!$isAdmin) {
            if ($settingsManager->getSetting('ticket.show_terms_if_profile_completed') === 'true') {
                $extraFieldValue = new ExtraFieldValue('user');
                $value = $extraFieldValue->get_values_by_handler_and_field_variable($userId, 'termactivated');
                if (isset($value['value'])) {
                    $termActivated = !empty($value['value']) && (int)$value['value'] === 1;
                }
    ...
    private function getExtraFieldBlock(
        ...
        $fieldVisibility = ($fieldVisibilityConfig && $fieldVisibilityConfig !== 'false') ? json_decode($fieldVisibilityConfig, true)['options'] : [];
