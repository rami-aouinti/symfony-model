...
use App\CoreBundle\Traits\GradebookControllerTrait;
...
class ResourceController extends AbstractResourceController implements CourseControllerInterface
{
    ...
    public function view(Request $request, TrackEDownloadsRepository $trackEDownloadsRepository): Response
    {
        ...
        $resourceNode = $this->getResourceNodeRepository()->findOneBy([
            'uuid' => $id,
        ]);
    ...
    public function link(Request $request, RouterInterface $router, CLinkRepository $cLinkRepository): RedirectResponse
    {
        ...
        if ($tool === 'course_tool' && $type === 'links') {
            $cLink = $cLinkRepository->findOneBy([
                'resourceNode' => $resourceNode,
            ]);
            if ($cLink) {
                $url = $cLink->getUrl();
        ...
        switch ($mode) {
            ...
            default:
                ...
                if (str_contains($mimeType, 'html')) {
                    ...
                    if ($this->getSettingsManager()->getSetting('editor.translate_html') === 'true') {
                        $user = $this->userHelper->getCurrent();
                        if ($user !== null) {
                    ...
                    if ($this->getSettingsManager()->getSetting('course.enable_bootstrap_in_documents_html') === 'true') {
