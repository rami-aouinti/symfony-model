...
use App\CoreBundle\Repository\ExtraFieldValuesRepository;
...
use App\CoreBundle\Security\Authorization\Voter\CourseVoter;
use App\CoreBundle\Service\CourseService;
...
use App\CourseBundle\Controller\ToolBaseController;
...
use App\CourseBundle\Settings\SettingsFormFactory;
...
class CourseController extends ToolBaseController
{
    ...
    public function checkTermsAndConditionJson(
        ...
        if (
            ...
            && $settingsManager->getSetting('registration.allow_terms_conditions') === 'true'
            && $settingsManager->getSetting('platform.load_term_conditions_section') === 'course'
        ) {
            ...
            if ($termAndConditionStatus === false) {
                $request->getSession()->set('term_and_condition', [
                    'user_id' => $user->getId(),
                ]);
                ...
                if (
                    $settingsManager->getSetting('course.allow_public_course_with_no_terms_conditions') === 'true'
                    && $course->getVisibility() === Course::OPEN_WORLD
                ) {
                    $redirect = false;
                }
    ...
    public function indexJson(
        ...
        if (
            $user && $isSpecialCourse && (isset($_GET['autoreg']) && (int)$_GET['autoreg'] === 1)
    ...
    public function getToolIntro(Request $request, Course $course, EntityManagerInterface $em): Response
    {
        ...
        if ($ctool) {
            ...
            $ctoolintro = $ctoolintroRepo->findOneBy([
                'courseTool' => $ctool,
            ]);
            if ($ctoolintro) {
                $responseData = [
    ...
    public function checkEnrollments(EntityManagerInterface $em, SettingsManager $settingsManager): JsonResponse
    {
        ...
        if (!$isEnrolledInCourses && !$isEnrolledInSessions) {
            ...
            $isEnrolledInCourses = $defaultMenuEntry === 'my_courses';
            $isEnrolledInSessions = $defaultMenuEntry === 'my_sessions';
        }
    ...
    public function getCategories(
        ...
        $allowBaseCourseCategory = $settingsManager->getSetting('course.allow_base_course_category') === 'true';
    ...
    private function autoLaunch(): void
    {
        ...
        if (api_get_setting('exercise.allow_exercise_auto_launch') === 'true') {
