...
namespace App\CoreBundle\Serializer;

use App\CoreBundle\Entity\Gradebook\GradebookCertificate;
use App\CoreBundle\Entity\Gradebook\GradebookResult;
use App\CoreBundle\Entity\Skill\SkillRelUserComment;
use App\CoreBundle\Entity\Ticket\Ticket;
use App\CoreBundle\Entity\Ticket\TicketMessage;
use App\CoreBundle\Entity\User\User;
use App\CoreBundle\Entity\User\UserCourseCategory;
use App\CoreBundle\Entity\User\UserRelCourseVote;
use App\CoreBundle\Repository\Node\UserRepository;
use App\CourseBundle\Entity\Attendance\CAttendanceResult;
use App\CourseBundle\Entity\Attendance\CAttendanceSheet;
use App\CourseBundle\Entity\CLp\CLpView;
use App\CourseBundle\Entity\CNotebook;
use App\CourseBundle\Entity\CWiki;
use App\CourseBundle\Entity\Dropbox\CDropboxFeedback;
use App\CourseBundle\Entity\Dropbox\CDropboxFile;
use App\CourseBundle\Entity\Dropbox\CDropboxPerson;
use App\CourseBundle\Entity\Group\CGroupRelUser;
...
final class UserToJsonNormalizer
{
    ...
    public function getPersonalDataToJson(int $userId, array $substitutionTerms): string
    {
        ...
        foreach ($result as $item) {
            ...
            $endDate = $item->getLogoutCourseDate() !== null ? $item->getLogoutCourseDate()->format($dateFormat) : '';
            $list = [
                'IP: ' . $item->getUserIp(),
                'Start: ' . $startDate,
                'End: ' . $endDate,
        ...
        foreach ($trackResults[TrackELogin::class] as $item) {
            ...
            $endDate = $item->getLogoutDate() !== null ? $item->getLogoutDate()->format($dateFormat) : '';
            $list = [
                'IP: ' . $item->getUserIp(),
                'Start: ' . $startDate,
                'End: ' . $endDate,
    ...
    public function serializeUserData(int $userId): string
    {
        ...
        $gradebookCertificates = $em->getRepository(GradebookCertificate::class)->findBy([
            'user' => $userId,
        ]);
        ...
        $trackEExercises = $em->getRepository(TrackEExercise::class)->findBy([
            'user' => $userId,
        ]);
        ...
        if ($resourceLinkId !== null) {
            $trackEDownloads = $em->getRepository(TrackEDownloads::class)->findBy([
        ...
        // UserCourseCategory
        $userCourseCategories = $em->getRepository(UserCourseCategory::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CForumPost
        $cForumPosts = $em->getRepository(CForumPost::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CForumThread
        $cForumThreads = $em->getRepository(CForumThread::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CGroupRelUser
        $cGroupRelUsers = $em->getRepository(CGroupRelUser::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CAttendanceSheet
        $cAttendanceSheets = $em->getRepository(CAttendanceSheet::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CAttendanceResult
        $cAttendanceResults = $em->getRepository(CAttendanceResult::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CSurveyAnswer
        $cSurveyAnswers = $em->getRepository(CSurveyAnswer::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CLpView
        $cLpViews = $em->getRepository(CLpView::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CStudentPublication
        $cStudentPublications = $em->getRepository(CStudentPublication::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CStudentPublicationComment
        $cStudentPublicationComments = $em->getRepository(CStudentPublicationComment::class)->findBy([
            'user' => $userId,
        ]);
        ...
        // CWiki
        $cWikis = $em->getRepository(CWiki::class)->findBy([
            'userId' => $userId,
        ]);
        ...
        // TicketMessage
        $ticketMessages = $em->getRepository(TicketMessage::class)->findBy([
            'insertUserId' => $userId,
        ]);
        ...
        // SkillRelUserComment
        $skillRelUserComments = $em->getRepository(SkillRelUserComment::class)->findBy([
            'feedbackGiver' => $userId,
        ]);
        ...
        // UserRelCourseVote
        $userRelCourseVotes = $em->getRepository(UserRelCourseVote::class)->findBy([
            'user' => $userId,
        ]);
