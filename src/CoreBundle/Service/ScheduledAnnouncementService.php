...
use App\CoreBundle\Repository\ScheduledAnnouncementRepository;
use App\CoreBundle\Repository\SessionRepository;
use App\CoreBundle\ServiceHelper\MessageHelper;
...
class ScheduledAnnouncementService
{
    ...
    public function sendPendingMessages(int $urlId, bool $debug = false): int
    {
        ...
        $announcements = $this->announcementRepository->findBy([
            'sent' => false,
        ]);
        ...
        $extraFields = $extraField->get_all([
            'filter = ? AND visible_to_self = ?' => [1, 1],
        ]);
    ...
    {
        return $this->settingsManager->getSetting('announcement.allow_scheduled_announcements') === 'true';
    }
