...
use App\CoreBundle\Entity\User\UsergroupRelUser;
 ...
 *
 * @package App\CoreBundle\Repository\Node
...
class UserRepository extends ResourceRepository implements PasswordUpgraderInterface
{
     ...
     * @return User[]
     */
    public function findByRole(string $role, string $keyword, int $accessUrlId = 0): array
    {
        $qb = $this->createQueryBuilder('u');
     ...
     * @return mixed
     */
    public function findByRoleList(array $roleList, string $keyword, int $accessUrlId = 0): mixed
    {
        $qb = $this->createQueryBuilder('u');
     ...
     */
    public function getCoachesForSessionCourse(Session $session, Course $course): Collection|array
    {
        $qb = $this->createQueryBuilder('u');
     ...
     * @return array
     */
    public function getSessionAdmins(User $user): array
    {
        $qb = $this->createQueryBuilder('u');
        $qb
            ->distinct()
     ...
     * @return int
     */
    public function getCountTeachersByUrl(AccessUrl $url): int
    {
        $qb = $this->createQueryBuilder('u');
     ...
     * @param string|null $searchFilter  Optional. The search text to filter the user list
     ...
     *
     */
    public function findUsersToSendMessage(int $currentUserId, ?string $searchFilter = null, int $limit = 10): array
    {
        ...
        $messageTool = api_get_setting('allow_message_tool') === 'true';
        if (!$messageTool) {
            return [];
        }
    ...
    public function getExtraUserData(int $userId, bool $prefix = false, bool $allVisibility = true, bool $splitMultiple = false, ?int $fieldFilter = null): array
    {
        ...
        foreach ($results as $row) {
            ...
            if ($splitMultiple && $row['type'] === ExtraField::USER_FIELD_TYPE_SELECT_MULTIPLE) {
                $value = explode(';', $value);
            }
    ...
    public function getExtraUserDataByField(int $userId, string $fieldVariable, bool $allVisibility = true): array
    {
        ...
        foreach ($result as $row) {
            ...
            if ($row['valueType'] == ExtraField::USER_FIELD_TYPE_SELECT_MULTIPLE) {
    ...
    public function getUserPicture(
        ...
        $width = match ($size) {
            self::USER_IMAGE_SIZE_SMALL => 32,
            self::USER_IMAGE_SIZE_MEDIUM => 64,
            self::USER_IMAGE_SIZE_BIG => 128,
            default => 0,
        };
    ...
    protected function reassignUserResourcesToFallback(User $userToDelete, User $fallbackUser): void
    {
        ...
        $userResourceNodes = $em->getRepository(ResourceNode::class)->findBy([
        ...
        $childResourceNodes = $em->getRepository(ResourceNode::class)->findBy([
            'parent' => $userToDelete->getResourceNode(),
        ]);
        ...
        $relations = [
            ...
            [
                'bundle' => 'CourseBundle',
                'entity' => 'CStudentPublicationComment',
                'field' => 'user',
                'type' => 'object',
                'action' => 'delete',
            ],
            [
                'bundle' => 'CourseBundle',
                'entity' => 'CStudentPublicationRelUser',
                'field' => 'user',
                'type' => 'object',
                'action' => 'delete',
            ],
            ...
            [
                'bundle' => 'CoreBundle',
                'entity' => 'TicketMessageAttachment',
                'field' => 'lastEditUserId',
                'type' => 'int',
                'action' => 'convert',
            ],
            [
            ...
            [
                'bundle' => 'CoreBundle',
                'entity' => 'TrackEExerciseConfirmation',
                'field' => 'user',
                'type' => 'object',
                'action' => 'delete',
            ],
        ...
        foreach ($relations as $relation) {
            $entityClass = 'App\\' . $relation['bundle'] . '\Entity\\' . $relation['entity'];
            $repository = $em->getRepository($entityClass);
            $records = $repository->findBy([
                $relation['field'] => $userToDelete,
            ]);
