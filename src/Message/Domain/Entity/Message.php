...
namespace App\Message\Domain\Entity;
...
use App\CoreBundle\Entity\Listener\MessageListener;
...
use App\CoreBundle\State\MessageByGroupStateProvider;
use App\CoreBundle\State\MessageProcessor;
use DateTime;
 ...
 * Class Message
 *
 * @package App\Message\Domain\Entity
...
class Message
{
    ...
    public function getReceiversTo(): array
    {
        return $this->receivers
            ->filter(
                fn (MessageRelUser $messageRelUser) => $messageRelUser->getReceiverType() === MessageRelUser::TYPE_TO
            )->getValues()
        ;
    }
    ...
    public function getReceiversCc(): array
    {
        return $this->receivers
            ->filter(
                fn (MessageRelUser $messageRelUser) => $messageRelUser->getReceiverType() === MessageRelUser::TYPE_CC
            )
            ->getValues()
        ;
    }
    ...
    public function getReceiversSender(): array
    {
        return $this->receivers
            ->filter(
                fn (MessageRelUser $messageRelUser) => $messageRelUser->getReceiverType() === MessageRelUser::TYPE_SENDER
            )
            ->getValues()
        ;
    }
