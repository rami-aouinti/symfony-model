...
use App\CoreBundle\Entity\Skill\SkillRelItem;
use App\CoreBundle\Entity\Skill\SkillRelItemRelUser;
...
switch ($action) {
    ...
    case 'update_skill_rel_user':
        ...
        if (!empty($typeId) && !empty($itemId) && !empty($skillId) && !empty($userId) && !empty($courseId)) {
            ...
            $skill = $em->getRepository(\App\CoreBundle\Entity\Skill\Skill::class)->find($skillId);
    ...
    case 'assign_user_to_skill':
        ...
        $em = Database::getManager();
        $skillRepo = $em->getRepository(\App\CoreBundle\Entity\Skill\Skill::class);
        ...
        $skillUserRepo = $em->getRepository(\App\CoreBundle\Entity\Skill\SkillRelUser::class);
        $criteria = [
            'user' => $user,
            'skill' => $skill,
        ];
        ...
        if (empty($skillRelUsers)) {
            $skillUser = new \App\CoreBundle\Entity\Skill\SkillRelUser();
