...
use App\CoreBundle\Entity\Language;
...
class SubLanguageManager
{
    ...
    public static function getIsoCodes($languageId)
    {
        ...
        $language = $em->getRepository('App\CoreBundle\Entity\Language')->find($languageId);
        ...
        if ($language->getParent()) {
            $parentLanguage = $em->getRepository('App\CoreBundle\Entity\Language')->find($language->getParent());
            if ($parentLanguage) {
    ...
    {
        $em = Database::getManager();
        $languageRepository = $em->getRepository('App\CoreBundle\Entity\Language');
